...
#2.1 -- tweeter une phrase aux 5 journalistes avec un hastag #bonjour_monde
five = journalists.sample(5)
handle = five.join(" ")
    five.each do |handle|
       
        client.update("#{handle}! Thank u for your tweet ! #bonjour_monde")
    end

#2.2 -- liker les derniers tweets (minimum les 25 derniers) qui ont le hashtag #bonjour_monde
like = client.search("#bonjour_monde", result_type: "recent").take(25)
client.favorite(like)

#2.3 -- follow les dernières personnes (minimum 20) qui ont tweeté avec le hashtag #bonjour_monde.
fol = client.follower_ids("#bonjour_monde", result_type: "recent").take(20)
client.follow(fol)

#2.4 -- utiliser la fonction Streaming de l'API Twitter afin de réagir en live. Mets en place un robot qui like et follow les comptes tweetant le #bonjour_monde
